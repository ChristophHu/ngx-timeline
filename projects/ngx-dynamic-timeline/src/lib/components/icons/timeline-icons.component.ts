...
import { TimelineIcons } from './timeline-icons';
...
export class TimelineIconsComponent {
    /**
   * The iconname to get from icons.ts to set to the innerHTML.
   * @type {string}
   */
  @Input() set name(iconName: string) {
    this.renderer.setProperty(this.element.nativeElement,'innerHTML', TimelineIcons[iconName] || null)
  }
